...
History:
  2022/03/28: start from binary template
  2022/03/29: continue
...
This tool takes XLSB files as input and parses them.
...
dRecordTypes = {
    ...
    2136: 'BrtSXDI15',
    2137: 'BrtBeginModelTimeGroupings',
    2138: 'BrtEndModelTimeGroupings',
    2139: 'BrtBeginModelTimeGrouping',
    2140: 'BrtEndModelTimeGrouping',
    2141: 'BrtModelTimeGroupingCalcCol',
    3072: 'BrtUID', # https://social.msdn.microsoft.com/Forums/azure/en-US/880eb3f3-395a-4097-ad01-8e0878d0b436/xlsb-brtuid-record-type-not-defined?forum=os_binaryfile
    3073: 'BrtRevisionPtr',
...
def ProcessBinaryFile(filename, content, cutexpression, flag, oOutput, oLogfile, options, oParserFlag):
    ...
    try:
        ...
        index = 1
        while len(data) > 0:
            recordType, data = GetInteger(data)
            recordSize, data = GetInteger(data)
            if recordSize > 0:
                recordData = data[:recordSize]
                data = data[recordSize:]
            else:
                recordData = b''
            recordTypeString = dRecordTypes.get(recordType, 'NOTFOUND(%d)' % recordType)
            line = '%3d: %s: %s' % (index, recordTypeString, recordSize)
            if recordTypeString in ['BrtFmlaString']:
                line = line + ' ' + XLWideStr(recordData[8:])
            elif recordTypeString in ['BrtSxvcellStr']:
                line = line + ' ' + repr(DumpFunctionStrings(recordData))
            elif recordTypeString in ['BrtSSTItem']:
                line = line + ' ' + RichStr(recordData)
            oOutput.Line(line)
            index += 1

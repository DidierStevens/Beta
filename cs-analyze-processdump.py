...
try:
    ...
    print('minidump module is required: pip install minidump')
    exit(-1)
...
def KeyStats(data):
    dStats = {}
    for byte in data:
        dStats[byte] = dStats.get(byte, 0) + 1
    return sorted(dStats.items(), reverse=True, key=lambda item: item[1])

def NormalizeKey(data):
    smallest = 0x100000000
    rotationSave = data
    for position in range(len(data)):
        rotation = data[position:] + data[:position]
        int32 = struct.unpack('>I', rotation[:4])[0]
        if int32 < smallest:
            smallest = int32
...
def AverageDifferenceConsecutiveBytes(data):
    return CalculateByteStatistics(data=data)[-1]
...
#                    print(len(bytesSegment))
        ...
        for dataInfo, data in listData:
            ...
            keysSorted = []
            normalizedKeys = set()
            for key, value in sorted(dKeys.items(), reverse=True, key=lambda item: item[1]):
                if value < 100:
                    continue
                keyStats = KeyStats(key)
                acbd = CalculateByteStatistics(data=key)[-1]
                if keyStats[0][1] < 4 and acbd >= 20:

...
def ProcessJPEGFile(filename, cutexpression, options):
    ...
    while True:
        ...
        if markerType == 0x00:
            ...
            if ffdaFlag:
                ...
                Print('                                  entropy-coded data: l=%d e=%f a=%f #ff00=%d' % ((found - endOfPrevious), stats[1], stats[10], ff00Counter), options)
                ff00Counter = 0
            ...
            if markerType >= 0xD0 and markerType <= 0xD9:
                ...
                line = '%3d p=0x%08x d=%d: m=%02x%02x %-5s l=%5d e=%f a=%f' % (counter, found, found - endOfPrevious, struct.unpack('Bx', marker[0:2])[0], markerType, markerName, size, stats[1], stats[10])
                if markerType == 0xDB:
    ...
    if trailing > 0:
        ...
        Print('Negative trailing', options)

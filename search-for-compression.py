...
This tool searches for ZLIB compresses data inside binary files. It goes sequentially through the binary data, starting at position 0 and tries to decompress the data at that position.
If it fails, it moves on to the next byte.
...
Value 0 means:
If decompression attempt fails, move on to the next byte.
If decompression attempt succeeds, move on to the first byte after the compressed data.
A value different from 0 means:
If decompression attempt succeeds, move on to the first byte after the compressed data provided that the compressed data is at least as large as the value provided with option -D.
Otherwise, move on to the next byte.

Option -j (--jsonoutput) produces JSON data containing all decompressed parts.

Decompressed data can be selected (-s) and dumped (-a, -A, -x, -X, -b, -B, -d).
    ...
    try:
        ...
        for iIter in range(recompressLengthStart, len(data)):
            try:
                ...
                if result == result2:
                    return result, iIter, data[iIter:]
            except:
                pass
        return result, -1, ''
    except:

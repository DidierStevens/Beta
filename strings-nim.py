...
__description__ = 'Extract strings for compiled Nim language programs'
...
History:
  2021/03/21: start based on binary template
...
This tool reads arbitrary files and extracts strings stored as Nim (programming language) strings: STRING_LITERAL.

By default, the tool extracts strings for 32-bit and 64-bit format. Option -t (--type) can be used to change this into 32-bit only (x86) or 64-bit only (x64).
...
def ProcessBinaryFile(filename, content, cutexpression, flag, oOutput, oLogfile, options, oParserFlag):
    ...
    try:
        ...
        formatx86 = '<II'
        formatx86Size = struct.calcsize(formatx86)
        formatx64 = '<QQ'
        formatx64Size = struct.calcsize(formatx64)
        for position in FindAll(data, b'\x40'):
            if (options.type == TYPE_OPTIONS[TYPE_OPTIONS_ALL] or options.type == TYPE_OPTIONS[TYPE_OPTIONS_X86]) and position >= formatx86Size - 1:
                len1, len2 = struct.unpack(formatx86, data[position - formatx86Size + 1:position + 1])
                if len2 - len1 == 0x40000000:
                    oOutput.Line(data[position + 1:position + 1 + len1].decode('utf8', 'surrogateescape'))
            if (options.type == TYPE_OPTIONS[TYPE_OPTIONS_ALL] or options.type == TYPE_OPTIONS[TYPE_OPTIONS_X64]) and position >= formatx64Size - 1:
                len1, len2 = struct.unpack(formatx64, data[position - formatx64Size + 1:position + 1])
                if len2 - len1 == 0x4000000000000000:
                    oOutput.Line(data[position + 1:position + 1 + len1].decode('utf8', 'surrogateescape'))
        # ----------------------------------------------
    except:
...
https://DidierStevens.com'''
    ...
    oParser.add_option('-t', '--type', default='all', help='Type of strings: %s (default %s)' % (' '.join(TYPE_OPTIONS), TYPE_OPTIONS[TYPE_OPTIONS_ALL]))
    ...
    if not options.type in TYPE_OPTIONS:
        oParser.print_help()
        oParserFlag.print_help()
        return

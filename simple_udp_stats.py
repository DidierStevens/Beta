...
__description__ = 'Process PCAP files to calculate UDP data statistics'
...
This tool processes PCAP files to calculate statistics of UDP data.

This Python script was developed with Python 2.7 and tested with Python 2.7 and 3.6.

All UDP data is grouped by connection ID: the combination of source IP, source port, destination IP and destination port.
The are 2 connection IDs per UDP connection: one per flow direction.
...
This tool can help identifying UDP connections that are not encrypted/encoded/compressed, as these connections will have a lower entropy.
    ...
    with PcapFile(filename, oLogfile) as fIn:
        try:
            for timestamp, buffer in dpkt.pcap.Reader(fIn):
                ...
                if not isinstance(frame.data, dpkt.ip.IP) or not isinstance(frame.data.data, dpkt.udp.UDP):
                    continue
                ipPacket = frame.data
                udpPacket = ipPacket.data

                connectionid = '%s:%d-%s:%d' % (IP2String(ipPacket.src), udpPacket.sport, IP2String(ipPacket.dst), udpPacket.dport)

...
__description__ = 'Tool for ISO files'
...
try:
    ...
    print('Missing module pathlab, install with: pip install pathlab')
    exit()
...
This tool reads one or more ISO files and list the contained files.

Option -s can be used to select and dump a file.
Dump options: -a, -A, -x, -d.
...
def ISOListFilesSub(ISOPath, result):
    for item in ISOPath.iterdir():
        if item.is_dir():
            ISOListFilesSub(item, result)
        elif item.is_file():
            result.append(item)

def ISOListFiles(oIsoAccessor):
    result = []
    ISOListFilesSub(oIsoAccessor.IsoPath('/'), result)
    return result
...
def ProcessBinaryFile(filename, content, cutexpression, flag, oOutput, oLogfile, options, oParserFlag):
    ...
    try:
        ...
        oFiles = ISOListFiles(pathlab.IsoAccessor(BufferedReader(DataIO(data))))
        if options.select == '':
            for index, oFile in enumerate(oFiles):
                index += 1
                oOutput.Line('%4d: %9d %s' % (index, oFile.stat().size, oFile.path))
        else:
            content = oFiles[int(options.select) - 1].read_bytes()
            if options.asciidump:
                oOutput.Line(cDump(content).HexAsciiDump())
            elif options.asciidumprle:
                oOutput.Line(cDump(content).HexAsciiDump(True))
            elif options.hexdump:
                oOutput.Line(cDump(content).HexDump())
            elif options.dump:
                oOutput.WriteBinary(content)
            else:
                oOutput.Line(cDump(content).HexAsciiDump())
        # ----------------------------------------------
    except:
